...
where
    T: Scalar,
{
    //println!("ndim: {}, depth {}, ext: {:?}", full_ndim, depth, tensor.index(0,0).extents());
    match tensor.extents() {
        &[len] => {
            let tensor_h = tensor.to_host();
            let data = tensor_h.buffer::<T>().unwrap();
            f.write_str("[")?;
...
impl fmt::Debug for Tensor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.data_type() {
            DataType::Char => format::<i8>(self, f, &fmt_opt),
            DataType::Short => format::<i16>(self, f, &fmt_opt),
            DataType::Int => format::<i32>(self, f, &fmt_opt),
            DataType::Long => format::<i64>(self, f, &fmt_opt),
            DataType::Uchar => format::<u8>(self, f, &fmt_opt),
            DataType::Ushort => format::<u16>(self, f, &fmt_opt),
            DataType::Uint => format::<u32>(self, f, &fmt_opt),
            DataType::Ulong => format::<u64>(self, f, &fmt_opt),
            DataType::Float => format::<f32>(self, f, &fmt_opt),
            DataType::Half => format::<f32>(self, f, &fmt_opt),
        }
    }
}
